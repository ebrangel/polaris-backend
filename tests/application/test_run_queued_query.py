...
from domain.models import Catalog, DatasourceType, QueryRequest, QueryStatus
from fakes import StubQueryExecutor


async def test_executa_no_perfil_heavy():
    stub = StubQueryExecutor()
    run = RunQueuedQuery(catalog=catalog(), executors={DatasourceType.POSTGRES: stub})
...
async def test_resolve_o_dataset_pelo_nome_nao_pela_cobertura():
    ...
    run = RunQueuedQuery(
        ...
        executors={DatasourceType.POSTGRES: postgres_stub, DatasourceType.ORACLE: oracle_stub},
    )
    request = QueryRequest(schema="vendas", dimensions=("sigla_uf",), measures=("valor_total",))
