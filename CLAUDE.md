...
- O **formato de saída** (JSON ou CSV) é negociado na borda HTTP e nunca entra no `QueryRequest`. `query_id` é o hash da requisição e serve de identificador do job assíncrono e nome do arquivo de export; se o formato fizesse parte dela, a mesma consulta em JSON e em CSV viraria dois `query_id` distintos — cache e execução duplicados por uma diferença que não muda o SQL gerado. Formato é representação do resultado, não parte da consulta, e por isso vive em querystring/header, nunca no corpo.
